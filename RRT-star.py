...
if __name__ == '__main__':
    ...
    for i in range(max_iterations):
        ...
        if ObstacleFree(node_nearest, node_new, obstacles):
            ...
            cost_min = G.nodes[node_nearest_id]["cost"] + LineCost(node_nearest, node_new)
            # Connect along a minimum-cost path
            for node_near_id in nodes_near_id:
                node_near = G.nodes[node_near_id]
            ...
            for node_near_id in nodes_near_id:
                ...
                if ObstacleFree(node_new, node_near, obstacles) and cost_new < node_near["cost"]:
                    parent_node = G.predecessors(node_near_id)
                    for j in parent_node:
                        parent_node_id = j

                    G.remove_edge(parent_node_id,node_near_id)
                    G.add_edge(i,node_near_id)
                    print(parent_node_id, node_near_id)
                    print(i, node_near_id)
                    print("\n")
